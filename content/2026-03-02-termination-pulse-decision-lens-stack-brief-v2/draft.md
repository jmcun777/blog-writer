...
1. Open case and run baseline across all six lenses.
2. Mark each lens green/amber/red with owner and due date.
3. Resolve red lenses before employee-facing actions.
4. Run legal + payroll parallel check for settlement/execution lenses.
5. Execute with live exception logging and change control.
6. Close only when execution and escalation lenses pass final gate.
7. Review weekly lens-failure trends to improve controls.
...
## What to do now
1. Deploy a one-page six-lens template for active cases.
2. Define red-lens criteria that force auto-hold.
3. Train managers on communication lens controls.
4. Require settlement worksheet co-sign before commitment.
5. Build weekly lens-failure dashboard by entity/city.
6. Recalibrate lens thresholds quarterly with local counsel.
...
## Operating cadence for lens governance
For foreign HQ teams, lens scoring works best with a fixed cadence. Run a weekly 30-minute lens review on active cases and a monthly 60-minute trend review across entities. Weekly review should answer three questions: which red lenses remain unresolved, which owners are overdue, and which cases need reroute decisions. Monthly review should focus on repeated lens failures, closure delays, and control changes that reduced escalations.

This cadence prevents the common problem of one-off checklists that are never revisited. It also improves cross-border alignment because teams in different time zones can follow the same decision rhythm and escalation protocol.

## Suggested KPI pack
Track a compact set of indicators:
- red-lens count by case stage
- red-lens aging over SLA
- script-drift incident rate
- settlement relock frequency
- closure lag (signature to verified closure)
- post-close reopen rate in 30 days

If two or more KPIs deteriorate for two consecutive cycles, trigger a control recalibration with HR, legal, and payroll owners.
